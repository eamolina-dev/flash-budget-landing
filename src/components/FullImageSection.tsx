...
export const FullImageSection = ({ src, alt = "" }: Props) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = Math.max(Math.min((window.innerHeight - rect.top) / (window.innerHeight + rect.height), 1), 0);
  ...
  return (
    <section className="relative w-full h-[72vh] md:h-[85vh] lg:h-screen overflow-hidden my-1">
      <img ref={imageRef} src={src} alt={alt} className="absolute inset-0 w-full h-full object-cover will-change-transform" />
